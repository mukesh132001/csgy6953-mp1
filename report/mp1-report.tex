...
which had previously been rejected for being prone to overfitting, paired with intense
...
intense augmentation and a cosine annealing scheduler.
The cosine annealing scheduler was selected for the intense augmentation model because the
loss curves of the learning rate scheduler experiment suggested that it minimizes training
loss at later epochs.
The intense augmentation model achieved validation accuracy of 94.4\%, while the default
augmentation model achieved TBD\%.
However, as shown in Figure~\ref{fig5}, at epochs beyond 40, some divergence of the training and validation loss curves
is evident, so it is likely that the intense augmentation method does not totally resolve
...
\begin{figure}[b]
\centering
\includegraphics[width=0.98\columnwidth]{experiment-tmp-80epoch-augtypes}
