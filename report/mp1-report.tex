...
was compared to configurations with no augmentation and with more intense augmentation,
adding random rotation of up to 30 degrees in either direction.
The loss curves for each of these configuration are shown in Figure~\ref{fig4}
...
While the model with more intense augmentation achieved only 88.0\% accuracy, its
validation loss curve tracks its training loss curve very closely.
Training loss for the intense augmentation model converges at a level higher than observed in
other experiments.
It was observed in the learning rate scheduler experiment that the scheduler has an impact on
how fast training loss is driven downward.
While reduction of training loss paired with stagnating validation loss is suboptimal,
if validation loss tracks training loss very closely, as it does for the model with intense augmentation,
minimizing training loss in the later epochs could be beneficial.

To explore this potential, the cosine annealing scheduler was employed, and a model was
trained to 80 epochs [to be continued].
