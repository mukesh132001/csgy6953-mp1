...
However, as shown in Figure~\ref{fig5}, at epochs beyond 40, the intense augmentation
...
That divergence and the similar validation accuracy scores suggest that the intense augmentation
method does not resolve overfitting problem.
The close tracking of training and validation loss was likely due to the intense
...
This model was trained to 80 epochs, with early stopping of training (at training accuracy 99.5\%).
The checkpoint with the best validation accuracy was selected to be evaluated on the test data.
On the CIFAR-10 test dataset, which contains 10k images, the model's accuracy is 94.0\%.
